...
![Screenshot of Set feature layer rendering mode on map sample](set-feature-layer-rendering-mode-on-map.png)
...
Use the 'Zoom In'/'Zoom Out' button to trigger the zoom animation on both static and dynamic maps.

## How it works

1. Create a map with operational layers and set the `FeatureRenderingMode` for each layer.
