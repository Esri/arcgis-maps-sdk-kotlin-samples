...
![Apply map algebra sample](apply-map-algebra.png)
...
3. Round elevation values down to the lowest 10-meter interval with map algebra operators
   `(continuousFieldFunction.div(10f)).floor().times(10f)`, and then convert the result to a `DiscreteFieldFunction` with
