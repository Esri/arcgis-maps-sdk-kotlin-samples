...
2. Create three `GraphicsOverlay`s as follows:  
   i. One overlay to hold the target's `Graphic`.  
   ii. One overlay to display a `Graphic` for each observer. Observers are in a separate `GraphicsOverlay` to allow us to detect when an observer graphic is tapped.  
   iii. One overlay to display the line of sight result graphics.
...
8. Create `Graphic`s from each result, using the geometry of the result's `visibleLine` and/or `notVisibleLine` properties, and appropriate symbols.
