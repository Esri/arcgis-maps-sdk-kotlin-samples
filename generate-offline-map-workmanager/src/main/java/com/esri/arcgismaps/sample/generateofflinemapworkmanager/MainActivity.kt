...
import com.esri.arcgismaps.sample.generateofflinemapworkmanager.databinding.ActivityMainBinding
import com.esri.arcgismaps.sample.generateofflinemapworkmanager.databinding.GenerateOfflineMapDialogLayoutBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private suspend fun displayOfflineMapFromJob(
        ...
        val notificationId = AtomicInteger(1).incrementAndGet()
        val workRequest = OneTimeWorkRequestBuilder<JobWorker>()
            ...
            .setInputData(
                workDataOf(
                    ...
                    jobParameter to offlineMapJob.toJson()
                )
            )
            .build()
//        var workRequestId : UUID = workRequest.id
        WorkManager.getInstance(applicationContext).enqueue(workRequest)

        val workInfoFlow: LiveData<WorkInfo> = WorkManager.getInstance(applicationContext)
